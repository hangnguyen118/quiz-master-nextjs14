...
import { browserLocalPersistence, createUserWithEmailAndPassword, GoogleAuthProvider, sendPasswordResetEmail, setPersistence, signInWithEmailAndPassword, signInWithPopup, updateProfile } from 'firebase/auth';
import { auth, db } from '@/_lib/firebase/firebase';
...
export default function Page() {
    ...
    const form = useForm({
        ...
        validate: {
            ...
            name: (val) => type === 'register' && (val.trim().length > 0 ? null : 'Invalid name'),
    ...
    const actions: Record<ActionType, (values: FormValues) => Promise<void> | void> = {
        async register(values: FormValues) {
            ...
            updateUserName(form.values.name);
            const formDataCopy = {
